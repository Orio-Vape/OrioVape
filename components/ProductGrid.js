function ProductGrid({ products, onAddToCart, onPreview, isAdmin, onEdit, onProductAdded, tableName }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (
                (tableName === 'products3' && (!formData.enablePrice1 || !formData.price1)) ||
                (
                    tableName !== 'products3' &&
            ...
            if (formData.featured && tableName !== 'products3') {
                await supabase.from('products').update({ featured: false }).neq('id', null);
            }
            let insertObj;
            if (tableName === 'products3') {
                // Solo los campos válidos para products3
                insertObj = {
                    name: formData.name,
    ...
    // Eliminar producto según la tabla
    const handleDeleteProduct = async (productId) => {
        if (!window.confirm('¿Eliminar este producto?')) return;
        let table = tableName || 'products';
        if (tableName === 'products3') {
            // Eliminar de products3
            const { error } = await supabase.from('products3').delete().eq('id', productId);
            if (!error && onProductAdded) onProductAdded();
        } else if (tableName === 'products2') {
            // Eliminar de products2
            const { error } = await supabase.from('products2').delete().eq('id', productId);
            if (!error && onProductAdded) onProductAdded();
        } else {
            // Eliminar de products (liquide)
                            ...
                            <>
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        onChange={handleChange}
                                    />
                                    <label htmlFor="enablePrice1" className="text-sm">Prix</label>
                                    <input
                                        type="number"
                                        name="price1"
                                        placeholder="Prix (MAD)"
    ...
    try {
        ...
        return (
            <section id={tableName === 'products3' ? 'concentre' : 'products'} data-name="product-grid" className="py-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {sortedProducts && sortedProducts.length > 0 ? (
                            sortedProducts.map(product => {
                                // NUEVO: lógica para mostrar el precio correcto según los precios habilitados
                                let displayPrice = '-';
                                if ((tableName === 'products2' || product.tableName === 'products2')) {
                                    // Jetables: muestra el primer precio habilitado en orden 0%, 3%, 5%
                                    if (product.enablePrice1 && product.price1 != null) {
                                        displayPrice = `MAD${product.price1}`;
                                    } else if (product.enablePrice2 && product.price2 != null) {
                                        displayPrice = `MAD${product.price2}`;
                                    } else if (product.enablePrice3 && product.price3 != null) {
                                        displayPrice = `MAD${product.price3}`;
                                    }
                                } else if (tableName === 'products3' || product.tableName === 'products3') {
                                    // Concentré: solo price1
                                    if (product.enablePrice1 && product.price1 != null) {
                                        displayPrice = `MAD${product.price1}`;
                                    }
                                } else {
                                    // Liquide: igual que antes, muestra price1 si está habilitado
                                    if (product.enablePrice1 && product.price1 != null) {
                                        displayPrice = `MAD${product.price1}`;
                                    } else if (product.enablePrice2 && product.price2 != null) {
                                        displayPrice = `MAD${product.price2}`;
                            ...
                            <div className="col-span-full text-center text-gray-500 py-10">
                                Il n'y a aucun produit disponible.
                            </div>
                        )}
                    </div>
                </div>
